...
//separate all individual images and crop them in order to obtain only an image of the tail
...
//Create a dialog box to ask if we want to manually erase the pigments
Dialog.create("Erase pigments");	
erase=getBoolean("Do you want to erase pigments manually on this image?");
if (erase==1){
run("Paintbrush Tool Options...", "brush=40");
setTool(19);
waitForUser("Are you done?", "Yes");
selectWindow("C1-"+title);
}
else if (erase==0){
selectWindow("C1-"+title);
